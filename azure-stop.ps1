# PowerShell script to stop Azure App Service
# Usage: .\azure-stop.ps1 -ResourceGroup <rg-name> -AppName <app-name>
...
Write-Host "  Stopping Azure App Service" -ForegroundColor Cyan
Write-Host "==========================================" -ForegroundColor Cyan
Write-Host ""
...
if ($state -eq "Stopped") {
    Write-Success "App is already stopped!"
    Write-Host ""
    Write-Info "No action needed - app is not running"
    Write-Host ""
    exit 0
}

# Stop the app
Write-Info "Stopping the app..."
az webapp stop --resource-group $ResourceGroup --name $AppName | Out-Null

Write-Success "App stopped successfully!"
Write-Host ""
...
if ($plans -and $plans.Count -gt 0) {
    $sku = $plans[0].sku.name
    
    if ($sku -eq "F1") {
        Write-Info "App is on Free Tier (F1) - stopping doesn't save costs"
    } else {
        Write-Success "App stopped - you're now saving money on the $sku tier!"
    }
}

Write-Host ""
Write-Info "To start the app again, run:"
Write-Info "  .\azure-start.ps1 -ResourceGroup $ResourceGroup -AppName $AppName"
Write-Host ""
Write-Host "==========================================" -ForegroundColor Cyan
